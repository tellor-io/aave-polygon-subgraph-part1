...
export class AccrueInterest extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type AccrueInterest must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
  ...
  static load(id: string): AccrueInterest | null {
    return changetype<AccrueInterest | null>(store.get("AccrueInterest", id));
  }
